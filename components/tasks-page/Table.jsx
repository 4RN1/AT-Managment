...
const Table = ({title, taskData, headingNumBgColor, headingNumColor}) => {
      ...
      <div className="my-5  border border-zinc-400  mx-10 rounded-lg">
        ...
        <div className=" border-b border-zinc-400">
          <h2 className="font-medium py-2 px-2">
            {title} <span 
            style={
              {background:headingNumBgColor  ,
               color:headingNumColor
            }}
  ...
  <div key={index}  className="flex justify-between items-center font-medium text-sm text-black px-2 border-t border-zinc-400 py-3 hover:bg-zinc-400/10 gap-5">
          <p className="w-full max-w-50">{task.uniqueCode}</p>
          <p className="w-full max-w-50">{task.title}</p>
          <button className="w-full max-w-50 py-1 bg-blue-500/10 text-blue-500 hover:text-blue-600 hover:bg-blue-600/10 rounded-lg cursor-pointer">ნახვა</button>
          <p className="w-full max-w-50">{task.assigned}</p>
          <p className="w-full max-w-50  bg-red-400/30 text-red-600 py-1 rounded-lg text-center">{task.priority}</p>
