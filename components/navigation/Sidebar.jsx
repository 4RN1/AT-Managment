...
const Sidebar = () => {
  return (
    <header className='w-22 h-full flex flex-col items-center justify-between bg-[#211C52] text-white py-5 px-2'>
        {/* logo */}
    ...
    <nav className='flex flex-col gap-3'>
      ...
      const Icon = nav.icon
return (
        <Link key={nav.link} href={nav.link} className='flex flex-col items-center text-xs px-1  py-1.5 rounded-md hover:bg-[#6150E1]'><Icon size={25} className='mb-1.5' />{nav.name}</Link>
      )})}
